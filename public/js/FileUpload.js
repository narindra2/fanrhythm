...
var FileUpload = {
    ...
    initDropZone:function (selector,url, isChunkUpload = false) {
        ...
        FileUpload.myDropzone = new Dropzone(selector, {
            ...
            addRemoveLinks: true,
            dictRemoveFile: "x",
            acceptedFiles: ".mp4,.mkv,.avi,.mov,.flv,.wmv,.webm,.mpeg,.3gp,.png,.jpg,.jpeg,.wav,.mp3",
            ...
            init: function() {
                ...
                FileUpload.attachaments.map((element)=>{
                    ...
                    if (!element.isInModeration) {
                        checkIfBreakRuleOfModeration();
                    }
        ...
        FileUpload.myDropzone.on("success", (file, response) => {
            if(response.success){
                ...
                FileUpload.attachaments.push({attachmentID: response.attachmentID, path: response.path, type:response.type, thumbnail:response.thumbnail ,isInModeration : response.isInModeration});
        ...
        FileUpload.myDropzone.on("totaluploadprogress", function (progress) {
            ...
            // var elProgressText = document.getElementById("#progress-text");  // my progress bar
            if (elProgress === undefined || elProgress === null) return;
             // changing progress bar's length based on progress value
    ...
    initFilepond : function (selector,url, isChunkUpload = false) {
        let chunkSize = 1024;
        if(isChunkUpload){
            url = url.replace('/upload/','/uploadChunked/');
        }
        ...
        FilePond.create(inputElement,
                {
                    labelIdle: `
                    <div>
                        ${trans('Ajouter à votre publication')}    <span class="filepond--label-action">Browse</span>
                    </div>
    ...
    setPreviewSource: function (element, file, attachment) {
        ...
        $(element).attr('title',  trans('inavid'));
...
function imagePreview() {
    return `<div class="dz-image shadow">
        ...
        <div class="dz-details">attachementBreackModerationRulesMessage
...
function checkIfBreakRuleOfModeration  () {
    let  target = $("#attachementBreackModerationRulesMessage");
    let  addPostbtn = $("#addPostbtn");
    var  hasBreakRules= false;
    FileUpload.attachaments.map((element)=>{
         if (typeof element.isInModeration !== 'undefined' && element.isInModeration === false) {
            hasBreakRules = true;
            setTimeout(() => {
                element.delete(); 
            }, 3000);
    ...
    if (hasBreakRules) {
        ...
        addPostbtn.addClass("d-none")
    }else{
        target.addClass("d-none");
        addPostbtn.removeClass("d-none")
    }
}
