...
$(function () {
    ...
    const themeCookie = document.cookie.replace(/(?:(?:^|.*;\s*)app_theme\s*=\s*([^;]*).*$)|^.*$/, "$1");
    const changeThemeLabel = document.getElementById('label-theme-active');
    
    if (themeCookie === 'light') {
        document.body.classList.remove('dark_theme');
        document.body.classList.add('light_theme');
        localStorage.setItem('theme', 'light_theme');
        changeThemeLabel.innerText = 'Mode light';
    } else {
        document.body.classList.add('dark_theme');
        document.body.classList.remove('light_theme');
        localStorage.setItem('theme', 'dark_theme');
        changeThemeLabel.innerText = 'Mode dark';
    }
});
...
function initStickyComponent(component, stickyClass) {
    ...
    if ($(".main-wrapper").length) {
        ...
        if ($(".main-wrapper").offset().top < top) {
            ...
            $(component).removeClass(stickyClass);
            sticky = false;
        }
    }
    // If the main-wrapper element doesn't exist, remove the sticky class
    else {
        $(component).removeClass(stickyClass);
        sticky = false;
    }
...
/**
 * Go to register via UI redirect
 */
function goToRegister() {
    redirect(app.baseUrl + '/register');
}
...
themeToggleBtn.addEventListener("click", function () {
    ...
    if (themeToggleBtn.classList.contains("active")) {
        ...
        document.body.classList.add("light_theme");
        localStorage.setItem("theme", "light_theme");
        document.getElementById('label-theme-active').innerText = 'Mode light';
        const theme = document.cookie.replace(/(?:(?:^|.*;\s*)app_theme\s*=\s*([^;]*).*$)|^.*$/, "$1");
        const newTheme = 'light';
        document.cookie = `app_theme=${newTheme}; path=/`;
    } else {
        document.body.classList.add("dark_theme");
        document.body.classList.remove("light_theme");
        localStorage.setItem("theme", "dark_theme");
        document.getElementById('label-theme-active').innerText = 'Mode dark';
        const theme = document.cookie.replace(/(?:(?:^|.*;\s*)app_theme\s*=\s*([^;]*).*$)|^.*$/, "$1");
        const newTheme = 'dark';
        document.cookie = `app_theme=${newTheme}; path=/`;
    }
...
function copieClipboard (textToCopie = "" , targetCopiedSuccess = ""){
    let title = $(targetCopiedSuccess).attr('title') || $(targetCopiedSuccess).attr('data-original-title');
    try {
        navigator.clipboard.writeText(textToCopie)
        $(targetCopiedSuccess).attr('title', trans('Copied!')).tooltip('dispose').tooltip().tooltip('show');
        setTimeout(() => {
            $(targetCopiedSuccess).attr('data-original-title', title);
            $(targetCopiedSuccess).attr('title', title);
        }, 2000);
    } catch (error) {
        setTimeout(() => {
            $(targetCopiedSuccess).attr('title', title);
