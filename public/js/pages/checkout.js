...
$(function () {
	...
	$('#checkout-center').on('show.bs.modal', function (e) {
		...
		$('#checkout-amount').attr("disabled",true);
		if (type === 'tip' || type === 'chat-tip') {
			$('#checkout-amount').attr("disabled",false);
			...
			paymentDescription =
				trans('Unlock post for') + ' ' + app.currencySymbol + amount;
			checkout.togglePaymentProviders(
				true,
				checkout.oneTimePaymentProcessorClasses
			);
			...
			paymentDescription =
				trans('Join streaming now for') + ' ' + app.currencySymbol + amount;
			checkout.togglePaymentProviders(
				true,
				checkout.oneTimePaymentProcessorClasses
			);
			...
			paymentDescription =
				trans('Unlock message for') + ' ' + app.currencySymbol + amount;
			checkout.togglePaymentProviders(
				true,
				checkout.oneTimePaymentProcessorClasses
			);
...
var checkout = {
	...
	updatePaymentForm: function () {
		...
		$('#post-id').val(checkout.paymentData.post);
		...
		$('#paymentPhone').val(checkout.paymentData.phone);
	...
	getSelectedPaymentMethod: function () {
		...
		const digitalVirgo = $('.digital-virgo-payment-provider').hasClass(
			'selected'
		);
		const Paydunya = $('.paydunya-payment-provider').hasClass('selected');
	...
	validatePhoneField: function () {
		let phoneField = $('input[name="phoneNumber"]');
		checkout.paymentData.phone = phoneField.val();
	},
	/**
	...
	updatePaymentSummaryData: function () {
					...
					for (let i = 0; i < countryTaxes.length; i++) {
						...
						if (countryTaxPercentage !== null && countryTaxPercentage > 0) {
							...
							if (countryTaxes[i].type === 'exclusive') {
								countryExclusiveTaxesPercentage +=
									parseFloat(countryTaxPercentage);
								taxes.push({
								...
								countryInclusiveTaxesPercentage +=
									parseFloat(countryTaxPercentage);
								taxes.push({
		...
		$('.available-credit').html(
			'(' + app.currencySymbol + checkout.paymentData.availableCredit + ')'
		);
		...
		$('.subtotal-amount b').html(
			app.currencySymbol + subtotalAmount.toFixed(2)
		);
