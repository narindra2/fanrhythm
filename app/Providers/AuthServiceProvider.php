...
class AuthServiceProvider extends ServiceProvider
{
    ...
    const ID_USER_COMMUNICATION = 231;
    ...
    public static function createUser($data)
    {
        ...
        try {
            ...
            $userData['referral_code_type_user'] = $code ."USER" ;
            ...
            if(getSetting('referrals.enabled')) {
                ...
                if(Cookie::has('referral')){
                    ...
                    if($referralID){
                        ...
                        if(!$existing) {
                            ReferralCodeUsage::create(['used_by' => $user->id,"referral_code_type" => User::REFERRAL_CODE_TYPE_MODEL, 'referral_code' => $referralID->referral_code]);
                ...
                if(Cookie::has('referral_code_type_user')){
                    $referral_code = Cookie::get('referral_code_type_user');
                    $referralUserID =  User::where('referral_code_type_user',$referral_code  )->first();
                    if($referralUserID){
                        $has = ReferralCodeUsage::where(['used_by' => $user->id, 'referral_code' => $referral_code ])->first();
                        if(!$has) {
                            ReferralCodeUsage::create(['used_by' => $user->id, "referral_code_type" => User::REFERRAL_CODE_TYPE_USER, 'referral_code' => $referral_code ]);
                            Cookie::queue(Cookie::forget('referral_code_type_user'));
                        }
                    }
                }
            }
            ListsHelperServiceProvider::managePredefinedUserMemberList($user->id, self::ID_USER_COMMUNICATION, ListsHelperServiceProvider::getUserFollowingType(self::ID_USER_COMMUNICATION));
        }
