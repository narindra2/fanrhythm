...
class PaymentHelper
{
    ...
    public function generateDigitalVirgoDepositTransaction($transaction)
    {
    }

    public function generateDigitalVirgoSubscriptionByTransaction($transaction)
    {
        $existingSubscription = $this->getSubscriptionBySenderAndReceiverAndProvider(
            $transaction['sender_user_id'],
            $transaction['recipient_user_id'],
            Transaction::DIGITAL_VIRGO
        );
        if ($existingSubscription != null) {
            $subscription = $existingSubscription;
        } else {
    ...
    public function generatePaydunyaSubscriptionByTransaction($transaction)
    {
        $existingSubscription = $this->getSubscriptionBySenderAndReceiverAndProvider(
            $transaction['sender_user_id'],
            $transaction['recipient_user_id'],
            Transaction::PAYDUNYA_PROVIDER
        );
        if ($existingSubscription != null) {
            $subscription = $existingSubscription;
        } else {
    ...
    public function generateOneTimeCreditTransaction($transaction)
    {
        ...
        if ($transaction['amount_without_free'] <= $userAvailableAmount) {
            $transaction['status'] = Transaction::APPROVED_STATUS;
        }
    }
        ...
        if ($transaction != null) {
            ...
            if ($transaction->status === Transaction::APPROVED_STATUS) {
                ...
                if ($this->isSubscriptionPayment($transaction->type)) {
                    ...
                    $successMessage = __('You successfully sent a tip of :currencySymbol:amount.', ['amount' => $transaction->amount_without_free, 'currencySymbol' => SettingsServiceProvider::getWebsiteCurrencySymbol()]);
                } elseif ($transaction->type === Transaction::POST_UNLOCK) {
