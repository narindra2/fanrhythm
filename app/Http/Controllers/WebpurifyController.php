   ...
   public function callbackReturnHandle($resp = [] , $id = 0)   {
    try {
        if (isset($resp["rsp"]["customvidid"]) && isset($resp["rsp"]["frames"]["frames"]) ) {
            ...
            if (Moderation::isVideoRespectTheModeration($frames , $rules)) {
                $status = Moderation::STATUS_APPROVED;
            }
            Attachment::where("id" ,$custom_id)->update(["moderation_status" =>$status]);
            DB::table("attachments_moderation_callback")->where("id",$id)->update(["status" => "done" , "attachment_id" =>   $custom_id  , "rules" => serialize($rules),"is_actived_verification_moderation" => getSetting("moderations.moderation_status")]);
        }
    } catch (\Throwable $th) {
