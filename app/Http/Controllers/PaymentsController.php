...
class PaymentsController extends Controller
{
    ...
    public function initiatePayment(CreateTransactionRequest $request)
    {
        ...
        try {
         ...
         //   return $request->has("phone"); 
            ...
            $data = [];
            if ($transaction['payment_provider'] == Transaction::DIGITAL_VIRGO) {
                $data = (new DigitalVirgoService)->generatePaymentUrl((int) $transaction['amount']);
            ...
            switch ($transactionType) {
                ...
                case Transaction::DEPOSIT_TYPE:
                    ...
                    if ($transaction['payment_provider'] == Transaction::PAYPAL_PROVIDER) {
                        ...
                        $redirectLink = $this->paymentHandler->generatePaystackTransaction($transaction, Auth::user()->email);
                    } elseif ($transaction['payment_provider'] == Transaction::DIGITAL_VIRGO) {
                ...
                case Transaction::YEARLY_SUBSCRIPTION:
                    ...
                    if ($transaction['payment_provider'] == Transaction::PAYPAL_PROVIDER) {
                        ...
                        $this->paymentHandler->generateDigitalVirgoSubscriptionByTransaction($transaction);
                    }
                    elseif ($transaction['payment_provider'] == Transaction::PAYDUNYA_PROVIDER) {
                        $this->paymentHandler->generatePaydunyaSubscriptionByTransaction($transaction);
                    }
                    break;
                default:
            ...
            $transaction['transaction_fees']  = getSetting("payments.transaction_fees");
            $transaction['amount']  =  $transaction['amount'] - Transaction::getTransactionFees($transaction);
            $transaction->save();

            if ( $transaction['payment_provider'] === Transaction::CREDIT_PROVIDER && $transaction['status'] === Transaction::APPROVED_STATUS
            ) {
            ...
            if ($transaction['payment_provider'] === Transaction::DIGITAL_VIRGO) {
                try {
                    header("Location: " . $data["data"]["payment_url"], true);
                    exit();
    ...
    public function updateUserBillingDetails($request)
    {
        ...
        if ($firstName != null || $lastName != null || $billingAddress != null || $phone != null) {
            ...
            if ($loggedUser != null) {
                ...
                if ($phone != null && $phone != $loggedUser->phone) {
