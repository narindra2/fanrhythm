...
class MessengerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        Javascript::put([
            'messengerVars' => [
                ...
                'lockedMessageSVGPath' => asset('/img/no-post.png'),
    ...
    public function sendMessage(Request $request)
    {
        ...
        try {
           if(getSetting("moderations.moderation_status") == "1" ){
               $result  =  Moderation::launchVerificationText($messageValue);
               if(isset($result["error"])){
                    return ['success' => false, 'errorModerationMessage' => $result["error"] ];
               }elseif(isset($result["foundCount"]) &&  $result["foundCount"]){
                return ['success' => false, 'errorModerationMessage' => __("L'écrit")  ." "  . collect($result["badText"])->map(function ( $item ) {return '«'   . $item  .'»' ; })->implode(", "). " " . __('ne est/sont pas permis selon notre règle de modération.')  ] ;
               }
           }
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'errors' => [$e->getMessage()], 'message'=> $e->getMessage()], 500);
        }
        ...
        if($message->hasUserUnlockedMessage == false && ($message->price && $message->price > 0) && $message->sender->id !== Auth::user()->id){
            ...
            foreach($message->attachments as $attachment){
                    if ($attachment->attachmentType == "video") {
                        $video .= "- " .  __("duration video") ." : " . $attachment->videoDuration ."<br>";
                    }elseif($attachment->attachmentType == "image"){
                        $nbImage++;
                    }else {
                        $nbOtherFile++;
                    }
            ...
            if($nbImage){
                $attchmntInfoPreview.=  "- $nbImage " . __("image(s)") . "<br>";
            }
            if($video){
                $attchmntInfoPreview.=  $video;
            }
            if($nbOtherFile){
                $attchmntInfoPreview .=  "- $nbOtherFile " . __("autre(s) fichier(s)"). "<br>";
            }
            unset($message->attachments);
            $message->attachments = collect([]);
            $message->attchmntInfoPreview = $attchmntInfoPreview;
        }
