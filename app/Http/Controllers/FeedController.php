...
class FeedController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $postsForUser231 = \DB::table('posts')
        ->leftJoin('attachments', 'posts.id', '=', 'attachments.post_id')
        ->leftJoin('users', 'posts.user_id', '=', 'users.id')
        ->where('posts.user_id', 231)
        ->select('posts.*','users.username','users.avatar', 'attachments.filename as attachment_filename', 'attachments.id as attachment_id')
        ->orderBy('posts.created_at', 'desc')
        ->get();
        
        return view('pages.feed', [
            'posts' => $posts, // Vos posts existants
            'suggestions' => MembersHelperServiceProvider::getSuggestedMembers(),
            'postsForUser231' => $postsForUser231, // Ajout des données filtrées
        ]);
    }
