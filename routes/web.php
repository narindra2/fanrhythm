...
*/
// test webpurify
Route::get('/webpurify/test',  function () {
   
});

//Set the referral code user in referral_code_type_user colone
Route::get('/set_referral_user/type',  function () {
    $users = User::all();
    foreach ($users as $user ) {
        try {
            $user->update(["referral_code_type_user" => $user->referral_code ."USER" ]);
        } catch (\Throwable $th) {
            continue;
        }
    }
   return ["success" => true , "message" => "Set the referral code user in referral_code_type_user colone is  ok"];
});
...
// Term and confidentiality
Route::get('/agreement',['uses' => 'GenericController@agreement', 'as'   => 'agreement']);
Route::get('/guidelines',['uses' => 'GenericController@guidelines', 'as'   => 'guidelines']);
Route::get('/privacy',['uses' => 'GenericController@privacy', 'as'   => 'privacy']);
Route::get('/dmca',['uses' => 'GenericController@dmca', 'as'   => 'dmca']);
Route::get('/discosure-statement',['uses' => 'GenericController@discosureStatement', 'as'   => 'discosureStatement']);
...
Route::group(['middleware' => ['auth','verified','2fa']], function () {
    ...
    Route::group(['prefix' => 'my', 'as' => 'my.'], function () {
        ...
        // (My) Card billing routes
        Route::post('/settings/add/card-billing', ['uses' => 'SettingsController@saveCardBillingUser' , 'as'   => 'settings.save.cardbillingUse']);
        Route::post('/settings/active/card-billing', ['uses' => 'SettingsController@activeCardBillingUser' , 'as'   => 'settings.active.cardbillingUse']);
    ...
    Route::group(['prefix' => 'attachment', 'as' => 'attachment.'], function () {
        ...
        Route::post('/upload2/{type}', ['uses' => 'AttachmentController@upload2', 'as'   => 'upload2']);
...
Route::get('/verified_user', 'VerifiedUserController@index')->name('verified_user');
...
Route::get('/terms_and_services_agreement', function () {
    return view('pages.terms_and_services_agreement');
})->name('terms_and_services_agreement');
