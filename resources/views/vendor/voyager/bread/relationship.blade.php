...
<?php var_dump($options )?>
                        ...
                        <div>
                            @foreach($selected_values as $selected_value)
                                @if($relationshipField === 'payment_request_hasmany_attachment_relationship')
                                    <div class="identity-files-preview itove_2">
                                        <a href="{{asset(Storage::url($selected_value))}}">
                                            <img src="{{asset(Storage::url($selected_value))}}" class="admin-id-asset"/>
                                        </a>
                                    </div>
                                @else
                                    <div>{{ $selected_value }}</div>
                                @endif
                            @endforeach
                        </div>
                    @endif
    ...
    @if($relationshipField === 'payment_request_hasmany_attachment_relationship' && $query_res instanceof \App\Model\Attachment)
        @php
            $filePath = \App\Providers\AttachmentServiceProvider::getFilePathByAttachment($query_res);
            $extension = pathinfo($filePath, PATHINFO_EXTENSION);
            $url = asset($filePath);
            $headers = get_headers($url);
            $fileExists = strpos($headers[0], '200 OK') !== false;
        @endphp
        @if($extension === 'jpg' && $fileExists && !str_starts_with($filePath, 'https://web.fanrhythm.com/storage/posts'))
            <div class="identity-files-preview izy">
                <a href="{{ asset($filePath) }}" target="_blank">
                    <img src="{{ asset($filePath) }}" class="admin-id-asset"/>
                </a>
            </div>
        @endif
    @endif
