                    ...
                    <form role="form"
                        ...
                        @if($dataType->slug == 'user-verifies')
                            ...
                            @foreach($dataType->editRows as $row)
                                @if($row->field == 'files' && $dataTypeContent->{$row->field})
                                        <p class="control-label" for="name">{{__("Identification images preview")}}</p>
                        ...
                        @if (count($errors) > 0)
                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                <div class="@if($row->type == 'hidden') hidden @endif {{ $errors->has($row->field) ? 'has-error' : '' }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif>
                                    {{ $row->slugify }}
                        ...
                        <div class="afrifan_footer_button">
                            @section('submit-buttons')
                                <button type="submit" class="btn-primary afri_btn_2 save">{{ __('voyager::generic.save') }}</button>
                            @stop
