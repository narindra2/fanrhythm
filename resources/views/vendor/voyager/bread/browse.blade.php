...
@section('page_header')
    <div class="aff_paddin_dash">
        ...
        @can('add', app($dataType->model_name))
                <a href="{{ route('voyager.'.$dataType->slug.'.create') }}" class="btn-add-new">
            ...
            @can('edit', app($dataType->model_name))
                @if(!empty($dataType->order_column) && !empty($dataType->order_display_column))
                    <a href="{{ route('voyager.'.$dataType->slug.'.order') }}" class="btn-add-new">
                            ...
                            <form method="get" class="form-search">
                                <div id="search-input">
                                    ...
                                    <button type="submit">
                                     ...
                                     <input type="text"  placeholder="{{ __('voyager::generic.search') }}" name="s" value="{{ $search->value }}">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            <th class="th-{{ $loop->index }}">
                                                @if ($isServerSide && in_array($row->field, $sortableColumns))
                                                    <a href="{{ $row->sortByUrl($orderBy, $sortOrder) }}">
                                                @endif
                                                @if ($row->field =="created_at" && $dataType->slug == "users" )
                                ...
                                @foreach($dataTypeContent as $data)
                                    <tr>
                                        ...
                                        @foreach($dataType->browseRows as $row)
                                            ...
                                            <td class="td-{{ $loop->index }}">
                                                ...
                                                @elseif($row->type == 'text')
                                                    ...
                                                    <div class="afrifan_find_badge_value-{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}"><span></span>{{ mb_strlen( $data->{$row->field} ) > 200 ? mb_substr($data->{$row->field}, 0, 200) . ' ...' : $data->{$row->field} }}</div>
                                        ...
                                        <td class="no-sort no-click bread-actions">
                                            ...
                                            <div class="dropdown afrifan_admin_more_drop">
