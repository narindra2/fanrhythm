<div class="aff_edit_info_form">
    ...
    <div class="aff_refferal_info">
        <div>
        {{__('Gérer tous vos abonnements actifs et annulés sur Afrifan.')}}
        </div>
        <div>
        {{__('Vous pouvez facilement consulter, modifier ou annuler vos abonnements en quelques clics. Avoir un contrôle total sur vos abonnements vous assure une expérience utilisateur personnalisée et confortable.')}}
...
.mt-0.mt-md-3.mb-1.inline-border-tabs  *,
.table-wrapper *  {
    font-size: 14px;
}
            ...
            @foreach($subscriptions as $subscription)
                <div class="col d-flex align-items-center py-3 border-bottom">
                    <div class="col-3 col-md-3 text-truncate">
                        ...
                        <a href="{{route('profile',['username'=> $activeSubsTab == 'subscriptions' ? $subscription->creator->username : $subscription->subscriber->username])}}" class="text-dark-r mode-dark-primary">
                    ...
                    <div class="col-4 col-md-2 text-truncate text-center">{{isset($subscription->created_at) ? ($subscription->status == \App\Model\Subscription::CANCELED_STATUS ? '-' : $subscription->created_at->format('M d Y')) : '-'}} </div>
                    <div class="col-2 text-truncate d-none d-md-block text-center">{{isset($subscription->expires_at) ? ($subscription->status == \App\Model\Subscription::CANCELED_STATUS ? '-' : $subscription->expires_at->format('M d Y')) : '-'}}</div>
