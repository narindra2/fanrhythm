...
<div class="aff_post_block post-box" data-postid="{{ $post->id }}"
    @if ($post->moderationStatus == $Moderation::STATUS_PENDING) style ="opacity: 0.6;" @endif
    @if ($post->moderationStatus == $Moderation::STATUS_DECLINED) style ="opacity: 0.8;border: 1px red solid;" @endif>

    <div class="aff_header_post">
        <a href="{{ route('profile', ['username' => $post->user->username]) }}">
            <img src="{{ $post->user->avatar }}">
            {!! $post->user->getUserStatusHtml("27px") !!}
        ...
        <div>
            ...
            <a class="aff_post_date"
                @if (!Auth::check()) onclick="goToRegister()" @else
                onclick="PostsPaginator.goToPostPageKeepingNav({{ $post->id }},{{ $post->postPage }},'{{ route('posts.get', ['post_id' => $post->id, 'username' => $post->user->username]) }}')" @endif
    ...
    @if ($post->moderationStatus == $Moderation::STATUS_DECLINED)
        <div class="aff_post_text">
            <span class="text-danger "> *
                {{ __("Ce post ne respecte pas nos normes de modération et ne peut pas être
                            publié") }}</span>
        </div>
    @endif
    ...
    @php
        $hasUserUnlockedPost = PostsHelper::hasUserUnlockedPost($post->postPurchases);
        $hasAttachments = count($post->attachments);
        if ($post->is_public) {
            $notLockedPost = false;
        } else {
            $notLockedPost =
                (Auth::check() && Auth::user()->id !== $post->user_id && $post->price > 0 && !$hasUserUnlockedPost) ||
        ...
        <div>
            <div>
                ...
                @if (Auth::check())
                    <div class="react-button {{ PostsHelper::didUserReact($post->reactions) ? 'active' : '' }}"
                        ...
                        <svg width="21px" height="18px" viewBox="0 0 21 18" version="1.1"
                            ...
                            <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"
                                stroke-linecap="round" stroke-linejoin="round">
                                <g id="Group" transform="translate(-16.000000, -785.000000)" fill="#FA5454"
                                    stroke="#FA5454" stroke-width="2">
                                    <g id="coeur" transform="translate(17.000000, 786.000000)">
                                        <path
                                            d="M16.7073684,1.35578947 C15.8387165,0.486729657 14.6603268,-0.00153736522 13.4315789,-0.00153736522 C12.2028311,-0.00153736522 11.0244414,0.486729657 10.1557895,1.35578947 L9.26315789,2.24842105 L8.37052632,1.35578947 C6.56135774,-0.45337905 3.62811597,-0.453379028 1.81894742,1.35578952 C0.00977886632,3.16495807 0.00977884447,6.09819984 1.81894737,7.90736842 L2.71157895,8.8 L9.26315789,15.3515789 L15.8147368,8.8 L16.7073684,7.90736842 C17.5764282,7.03871646 18.0646953,5.86032676 18.0646953,4.63157895 C18.0646953,3.40283113 17.5764282,2.22444143 16.7073684,1.35578947 Z"
                                            id="Path"></path>
                ...
                @else
                    ...
                    <div class="disabled" {{-- data-toggle="modal"
                ...
                @if (Auth::check() && $post->user->id != Auth::user()->id)
                    ...
                    data-recipient-id="{{ $post->user_id }}"> @endif
                            ...
                            <g id="Symbols" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"
                                ...
                                <g id="Group" transform="translate(-18.000000, -784.000000)" stroke="#000000"
                                    ...
                                    <g id="gift" transform="translate(19.000000, 785.000000)">
                                        ...
                                        <line x1="7.675" y1="15.35" x2="7.675" y2="3.8375"
                                            id="Path"></line>
                                        <path
                        ...
                        {{-- BookMarks --}}
                        @if ($hasAttachments)
                            @if (($hasUserUnlockedPost && $post->price > 0) || ($post->price == 0 && $post->isSubbed))
                                @php
                                ...
                                <div class="bookmark-button {{ $isPostBookmarked ? 'active' : '' }} bookmark-button-{{ $post->id }}"
                                    id="bookmark-button-{{ $post->id }}" data-toggle="tooltip"
                                    data-placement="top"
                                    data-original-title="{{ $isPostBookmarked ? __('Retirer aux bookmarks.') : __('Ajouter aux bookmarks.') }}"
                                ...
                                <g id="Symbols" stroke="none" stroke-width="1" fill="none"
                                    ...
                                    <g id="Group" transform="translate(-18.000000, -784.000000)" stroke="#000000"
                                        ...
                                        <g id="gift" transform="translate(19.000000, 785.000000)">
                                            ...
                                            <rect id="Rectangle" x="0" y="3.8375" width="15.35" height="3.8375">
                                            </rect>
                                            <line x1="7.675" y1="15.35" x2="7.675" y2="3.8375"
                                                id="Path"></line>
                                            <path
            ...
            <div class="show-all-comments-label pl-3 d-none">
                ...
                @else
                    <a onClick="CommentsPaginator.loadResults({{ $post->id }});"
                        href="javascript:void(0);">{{ __('Show
                                        more') }}</a>
                @endif
            </div>
