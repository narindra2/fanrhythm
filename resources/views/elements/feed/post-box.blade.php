...
<div class="aff_post_block post-box" data-postid="{{$post->id}}" @if ($post->moderationStatus == $Moderation::STATUS_PENDING )
    ...
    <div class="post-media">
        @if($post->isSubbed || (getSetting('profiles.allow_users_enabling_open_profiles') && $post->user->open_profile))
            @if((Auth::check() && Auth::user()->id !== $post->user_id && $post->price > 0 && !$hasUserUnlockedPost) || (!Auth::check() && $post->price > 0 ))
                    ...
                    @if ($hasAttachments)
                        @if ( ($hasUserUnlockedPost && $post->price > 0 ) || ($post->price == 0 && $post->isSubbed) )
                            ...
                            <div class="bookmark-button {{ $isPostBookmarked  ? 'active' : ''}} bookmark-button-{{ $post->id }}"
                                id="bookmark-button-{{ $post->id }}"  data-toggle="tooltip" data-placement="top" data-original-title="{{  $isPostBookmarked ? __("Retirer aux bookmarks.") : __('Ajouter aux bookmarks.')}}"
