<button class="btn btn-outline-primary to-tooltip {{((Auth::check() && !GenericHelper::isEmailEnforcedAndValidated()) || (Auth::check() && !GenericHelper::creatorCanEarnMoney($user)) ) ? 'disabled' : ''}}"
        @if(Auth::check())
    ...
    @endif
>
    <span>{{__('Subscribe')}}  {{__('for')}} {{trans_choice('months', 3,['number'=>3])}}</span>
    <span>
        @if(($user->profile_access_price_3_months * 3) == 15)
            {{ $user->profile_access_price * 3 }}
        @else
            {{ $user->profile_access_price_3_months * 3 }}
        @endif

        {{config('app.site.currency_symbol') ? '' : ' ' .config('app.site.currency_code')}}
        {{config('app.site.currency_symbol') ?? config('app.site.currency_symbol')}}
    </span>
</button>
