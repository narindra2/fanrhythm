<button class="btn btn-primary to-tooltip {{((Auth::check() && !GenericHelper::isEmailEnforcedAndValidated()) || (Auth::check() && !GenericHelper::creatorCanEarnMoney($user)) ) ? 'disabled' : ''}}"
        @if(Auth::check())
    ...
    @endif
>
    <span>{{__('Subscribe')}} {{__('for')}} {{trans_choice('days', 30,['number'=>30])}}</span>
    <span class="d-sm-block">
       
        {{$user->profile_access_price}}
